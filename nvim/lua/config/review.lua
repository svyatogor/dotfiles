...
vim.api.nvim_create_user_command('ReviewAdd', function(opts)
  ...
  vim.api.nvim_create_autocmd('InsertLeave', {
    ...
    callback = function()
      ...
      if vim.fn.getline(from) == line then vim.api.nvim_buf_set_lines(0, from - 1, from, false, {}) end
    end,
  })
...
-- Diffview builds revision buffers with `buftype=nowrite`, which todo-comments skips outright.
vim.api.nvim_create_autocmd('User', {
  pattern = 'DiffviewDiffBufWinEnter',
  callback = function()
    local hl = require 'todo-comments.highlight'
    for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
      if vim.wo[win].diff then hl.attach(win, true) end
    end
    hl.update()
  end,
})
